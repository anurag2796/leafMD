...
class ModelRegistry:
    ...
    def compare(self, version1: str, version2: str):
        ...
        for key in sorted(metrics_keys):
            ...
            if isinstance(val1, (int, float)) and isinstance(val2, (int, float)):
                ...
                print(f"{key:<20} | {val1:<10.4f} | {val2:<10.4f} | {symbol} {diff_str}")
            else:
    ...
    def promote(self, version: str, to: str = 'production'):
        ...
        if to not in valid_stages:
            print(f"❌ Invalid stage: {to}. Choose from: {valid_stages}")
            return False
        ...
        print(f"✅ Promoted v{version} to {to}")
        
        if to == 'production':
            print(f"\n⚠️  PRODUCTION DEPLOYMENT")
            print(f"   Update mobile app to use: models/v{version}/best.mlpackage")
    ...
    def rollback(self, version: str):
        ...
        if current_prod == version:
            print(f"⚠️  v{version} is already in production")
            return False
        
        print(f"🔄 Rolling back from v{current_prod} to v{version}")
        
        # Interactive prompt generally avoided in automated CLI unless --yes is passed
        # For now, we will assume user intent if they call the command.
    ...
    def print_table(self, versions: List[Dict]):
        ...
        print(f"{'Version':<12} | {'mAP@50':<8} | {'Size':<8} | {'Status':<12} | {'Date':<12}")
        print("-" * 90)
        ...
        for v in versions:
            ...
            status = v['status']
            date = v['registered_at'][:10]  # Just the date part
            
            # Add emoji for production
            if status == 'production':
                status = f"🌟 {status}"
            elif status == 'staging':
                status = f"🚀 {status}"
            
            print(f"v{version:<11} | {map50:<8.4f} | {size:<8} | {status:<12} | {date:<12}")
