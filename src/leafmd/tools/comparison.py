...
class ModelComparator:
    ...
    def benchmark_model(
        ...
        val_device = EnvironmentValidator.get_validation_device(device)
        ...
        try:
            ...
            # Validation on safe device (CPU for MPS)
            logger.info(f"📊 Validating {model_name} on {val_device.upper()}...")
            metrics = model.val(device=val_device, split="test", verbose=False)
    ...
    def run_comparison(
        ...
        logger.info(
            f"Device: {device if device != 'auto' else 'auto-detect'}"
        )
        logger.info("")
    ...
    def generate_report(self):
        ...
        logger.info(
            ...
            f"(mAP@50-95: {best_accuracy['map50_95']:.4f})"
        )
        logger.info(
            f"   Inference: {fastest_inference['model']} "
            f"({fastest_inference['inference_ms']:.1f} ms)"
        )
        logger.info(
            f"   Training:  {fastest_training['model']} "
            f"({fastest_training['training_time_min']:.1f} min)"
        )
        ...
        if yolo11_results and yolo26_results:
            ...
            map_diff = (
                (yolo26["map50_95"] - yolo11["map50_95"])
                / yolo11["map50_95"]
                * 100
            )
            speed_diff = (
                (yolo26["inference_ms"] - yolo11["inference_ms"])
                / yolo11["inference_ms"]
                * 100
            )
            ...
            logger.info(f"\n📈 YOLO26 vs YOLO11:")
            logger.info(
                f"   Accuracy: {map_diff:+.1f}% "
                f"{'🟢 BETTER' if map_diff > 0 else '🔴 WORSE'}"
            )
            logger.info(
                f"   Speed:    {speed_diff:+.1f}% "
                f"{'🟢 FASTER' if speed_diff < 0 else '🔴 SLOWER'}"
            )
            ...
            if map_diff > 2 and speed_diff < 10:
                logger.info(
                    "   ✅ Use YOLO26 — Better accuracy, acceptable speed"
                )
                recommendation = "yolo26"
            elif map_diff < -2:
                logger.info(
                    "   ⚠️  Stick with YOLO11 — YOLO26 is less accurate"
                )
                recommendation = "yolo11"
            elif speed_diff > 20:
                logger.info(
                    "   ⚠️  Stick with YOLO11 — YOLO26 is too slow"
                )
                recommendation = "yolo11"
            else:
                logger.info(
                    "   ⚠️  Results are too close. Use YOLO11 (more mature)"
                )
                recommendation = "yolo11"
        else:
